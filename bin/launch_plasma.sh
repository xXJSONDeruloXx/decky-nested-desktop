...
function cleanup()
{
    ...
    umount --recursive $NEW_XDG_RUNTIME_DIR 2>/dev/null || true
    rm -Rf $NEW_XDG_RUNTIME_DIR
}

# Store the original XDG_RUNTIME_DIR
ORIGINAL_XDG_RUNTIME_DIR=${XDG_RUNTIME_DIR:-/run/user/$(id -u)}

# Create a new XDG_RUNTIME_DIR for the nested session
NEW_XDG_RUNTIME_DIR=$ORIGINAL_XDG_RUNTIME_DIR/nested_plasma
trap cleanup EXIT
...
ln -s $ORIGINAL_XDG_RUNTIME_DIR/pulse/native $NEW_XDG_RUNTIME_DIR/pulse/native 2>/dev/null || true
ln -s $ORIGINAL_XDG_RUNTIME_DIR/pipewire* $NEW_XDG_RUNTIME_DIR/. 2>/dev/null || true
